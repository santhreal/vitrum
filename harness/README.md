...
`axiomexec` is an i9-13900K with a different core count, a different clock and
...
- A remote number is **not** comparable to a figure in `GOAL.md`, and must not
  ...
  good. Until that exists, quote remote results as "on axiomexec, N windows,
...
The default is `axiomexec`, with `santhserver` as the fallback. Each is tried
...
As of this writing, `axiomexec` is an Ubuntu 24.04.3 box with an i9-13900K, 64
...
So the two hosts need opposite things: `axiomexec` has the tools and needs the
engine, `santhserver` has the engine and needs the tools. Either way it is one
`apt-get` line, and `probe` prints the right one for whichever host answered.

Treat `santhserver` as a **third** baseline, not as a spare copy of the second.
It has half the threads and half the memory of `axiomexec` and a different CPU
...
while this was being built and `axiomexec` sat at 0.04, which is most of the
...
- **Idle CPU is a percentage of a different core.** The desktop is a Ryzen 9
  9950X; `axiomexec` is an i9-13900K with a different clock, different boost
...
- **The operator-waiting state may be UNKNOWN there, and a capture cannot tell
  ...
  Measured, not predicted: shipping the vitrum-core test binary to `axiomexec`
  ...
  Run three ways, that gives: this desktop scope 1, OK. `axiomexec` scope 2,
  DENIED. `santhserver` scope 1, OK. So the two candidate hosts differ on this
  and `santhserver` does not have the problem at all, which is one more reason
...
`axiomexec@` and `santh@`, from `~/.ssh/config`.
...
Verified against `axiomexec`, and the probe additionally against
`santhserver`:
...
`axiomexec` lacks the engine, `santhserver` lacks the tooling. So the parts of
