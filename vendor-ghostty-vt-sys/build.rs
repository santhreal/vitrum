...
/// The CPU model zig compiles for, on every target, always.
///
/// DIVERGENCE FROM UPSTREAM, with the reasoning at the `-Dcpu` argument below.
/// `baseline` is zig's name for the lowest model of whatever architecture is
/// being built: x86_64-v1 on x86, armv8-a on aarch64. It is never `native`.
/// `tools/release/check-isa.sh` disassembles the published binaries and fails
/// on anything above this, so the two have to move together.
const ZIG_CPU: &str = "baseline";
...
fn build_vendored(link_mode: LinkMode) {
    ...
    // DIVERGENCE FROM UPSTREAM. Upstream passes -Dtarget only when
    // `target != host`, so a native build lets zig detect the builder's CPU
    // and compile for it. That is fine for a developer building for their own
    // machine and wrong for anything published: every release target here is
    // built natively, so the shipped instruction set became whatever CPU the
    // runner happened to have. Built on a Zen 5 machine it emitted AVX-512,
    // which is an illegal instruction on most of the CPUs the triple promises.
    //
    // Always pass the target, and always pass the CPU. Passing -Dtarget alone
    // would be enough today, because zig defaults an explicitly named target
    // to its baseline CPU, but that is a default rather than a promise; -Dcpu
    // says what we mean and survives a change in that default.
    //
    // `baseline` is the lowest CPU model for the architecture: x86_64-v1 for
    // x86, armv8-a for aarch64. Nothing here is hot enough to justify raising
    // the floor and dropping machines for it; the terminal parser is not where
    // this application spends its time. Raising it is a measurement away, and
    // must move `tools/release/check-isa.sh` in the same change.
    let zig_target = zig_target(&target);
    build.arg(format!("-Dtarget={zig_target}"));
    build.arg(format!("-Dcpu={ZIG_CPU}"));
    // `host` is read above for its other uses and is deliberately no longer
