    ...
    fn the_publish_list_names_a_crate_after_everything_it_depends_on() {
        ...
        for (crate_name, dep) in &edges {
            ...
            let Some(dep_at) = position(dep) else {
                // A member that sets `publish = false` is not what a consumer
                // resolves. `libghostty-vt-sys` is vendored here to pin the
                // compiler flags the shipped binaries are built with, and the
                // registry copy is what anyone building from crates.io gets,
                // so ordering it in the publish loop would upload a fork of
                // someone else's crate.
                let dep_dir = by_dir
                    .iter()
                    .find(|(_, package)| package == dep)
                    .map(|(dir, _)| dir.clone());
                let unpublished = dep_dir
                    .and_then(|dir| std::fs::read_to_string(root.join(dir).join("Cargo.toml")).ok())
                    .is_some_and(|text| {
                        text.lines().any(|line| line.replace(' ', "") == "publish=false")
                    });
                if unpublished {
                    continue;
                }
